 ...
 */
public class ColorManager implements DdPreferenceConstants
{
  /**
  ...
  public ColorManager()
  {
    ...
    try
    {
      IPreferenceStore preferenceStore = DdUtils.getPreferenceStore();
      if (m_nCurCompColorTable <0) m_nCurCompColorTable  = preferenceStore.getInt(P_VIS_COMPCOLORTAB );
      if (m_nCurValueColorTable<0) m_nCurValueColorTable = preferenceStore.getInt(P_VIS_VALUECOLORTAB);
    }
    catch (Throwable e)
    {
  ...
  protected void finalize() throws Throwable
  {
    ...
    if (--m_nRc == 0)
    {
      DdUtils.MSG("VisColorManager: cleaning up");
      disposeCompColors();
      disposeValueColors();
    }
