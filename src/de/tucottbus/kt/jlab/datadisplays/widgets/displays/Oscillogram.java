...
public class Oscillogram extends AbstractRvDataDisplay implements Playable 
{
  ...
  //       scrolling, overlaid markers, selections etc.
    ...
    boolean bigPoints = (size / shownRecords) > DdUtils.OSCI_POINT_DISTANCE;
    boolean bNan;
	...
	protected DisplayInfoMouseMoveEvent createInfoEventOnMouse(int x, int y)
  {
	...
	class Player implements Runnable, LineListener {
		...
		public void play(IAction iPlayAction) {
			...
			m_iSdl = SoundUtil.getPlaybackDataline(iAF,DdUtils.getPreferenceStore());
  ...
  /**
   * Reflects the play-back state at the GUI.
   * 
   * @param iPlayAction
   *          Handle to GUI controls reflecting the player's state, may be
   *          <code>null</code>.
   */
  public void updatePlayAction(IAction action)
  {
    ...
    String sMsg = "";
    if (isPlaying())
      PlayActionUtil.setStop(action);
    else
    {
      if (getSrate() != getPlaySrate())
        sMsg = "changing sample rate from " + (getSrate() / 1000.) + " kHz to "
            + (getPlaySrate() / 1000.) + " kHz";
      if (sMsg.length() == 0)
        PlayActionUtil.setPlay(action);
      else
        PlayActionUtil.setPlayWarning(action, sMsg);
    }
  }
