...
implements DdPreferenceConstants
{
  ...
  private int m_nSig = 0; 
	...
	) throws DataException 
	{
		...
		computeValueRange();
		getValues();
		m_nLevels = DdUtils.decimalZoning(m_nMinValue, m_nMaxValue, 10);
   ...
   *          the component index of the data value
    ...
    if (DdUtils.bSpecShowValues)
      if (ColorManager.getColorValue(m_aColors[col])<128)
        iFgColor = getDisplay().getSystemColor(SWT.COLOR_WHITE);

    // Paint level curve
    if (DdUtils.bSpecShowLevels)
	...
	protected void paintData(GC iGc, Rectangle iDamage, RulerCalculator iHrc, RulerCalculator iVrc)
  {
    ...
    DdUtils.MSG("- Spectrogram:");
    DdUtils.MSG("  components [" + m_nMinComp + "..." + m_nMaxComp + "]");
    DdUtils.MSG("  visible    [" + m_nFirstComp + "..." + m_nLastComp + "]");
    ...
    IPreferenceStore iStore = DdUtils.getPreferenceStore();
    if (!DdUtils.bSpecShowValues && !DdUtils.bSpecShowLevels) DdUtils.bSpecShowValues = true;
